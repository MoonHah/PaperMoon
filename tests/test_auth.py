"""认证端点测试：注册 / 登录 / me / 401 / 409。用 conftest 的 SQLite 内存库。"""
...
def test_login_then_me(client: TestClient):
    client.post("/api/v1/auth/register", json={"email": "x@y.com", "password": "secret123"})
    token = client.post(
...
def test_login_wrong_password(client: TestClient):
    client.post("/api/v1/auth/register", json={"email": "w@z.com", "password": "secret123"})
    resp = client.post(
...
def test_duplicate_email_rejected(client: TestClient):
    client.post("/api/v1/auth/register", json={"email": "d@d.com", "password": "secret123"})
    resp = client.post(
